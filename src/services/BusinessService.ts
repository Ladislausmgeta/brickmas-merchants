...
export class BusinessService {
  ...
  async createBusiness(
    ...
    if (files) {
      ...
      for (const f of fileList) {
        try {
          ...
          console.warn(
            "File scan failed or infected, skipping:",
            f.originalname,
            err
          );
        }
    ...
    if (files?.brellaCertificate)
      payload.brellaCertificate = await this.moveToPermanent(
        files.brellaCertificate[0].path
      );
    if (files?.businessLicense)
      payload.businessLicense = await this.moveToPermanent(
        files.businessLicense[0].path
      );
    if (files?.ownerNidaIdDoc)
      payload.ownerNidaIdDoc = await this.moveToPermanent(
        files.ownerNidaIdDoc[0].path
      );
  ...
  async updateBusiness(
    ...
    if (files?.brellaCertificate)
      existing.brellaCertificate = await this.moveToPermanent(
        files.brellaCertificate[0].path
      );
    if (files?.businessLicense)
      existing.businessLicense = await this.moveToPermanent(
        files.businessLicense[0].path
      );
    if (files?.ownerNidaIdDoc)
      existing.ownerNidaIdDoc = await this.moveToPermanent(
        files.ownerNidaIdDoc[0].path
      );
  ...
  async publish(id: string) {
    await this.repo.update(id, { isPublished: true, isSuspended: false });
  }
  async unpublish(id: string) {
    await this.repo.update(id, { isPublished: false });
  }
  async suspend(id: string) {
    await this.repo.update(id, { isSuspended: true, isPublished: false });
  }
  ...
  async list(filter?: { published?: boolean; suspended?: boolean }) {
    ...
    if (filter?.published !== undefined)
      qb.andWhere("b.isPublished = :p", { p: filter.published });
    if (filter?.suspended !== undefined)
      qb.andWhere("b.isSuspended = :s", { s: filter.suspended });
    return qb.getMany();
  }
}
